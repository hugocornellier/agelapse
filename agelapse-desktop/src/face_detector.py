...
def _get_iris_mask(results: IrisResults, iris_location: _Rect, iris_size: _Size,
                   image_size: _Size) -> Image.Image:
    ...
    y_start, y_end = int(max(bbox.ymin, top)), int(min(bbox.ymax, bottom))
...
class FaceDetector:
    ...
    def get_eyes_from_faces(self, faces, img):
        ...
        for face in faces:
            ...
            point = Point(x, y)
            both_eyes.append(point)

            x, y = self.getXY(img, right_eye_results)
            point = Point(x, y)
            both_eyes.append(point)

            eyes.append(both_eyes)
