...
void main() {
  ...
  tearDownAll(() async {
    await StabUtils.disposeCatDetector();
  ...
  Future<bool> initAppAndFixtures(WidgetTester tester) async {
    ...
    if (!fixturesLoaded) {
      ...
      if (!fixturesUnavailable) {
        testImage = await getSampleCatPathAsync(1);
      }
      fixturesLoaded = true;
    }
  ...
  group('Cat Detection', () {
    testWidgets('detects cat from bytes', (tester) async {
      if (!await initAppAndFixtures(tester)) return;

      final bytes = await File(testImage!).readAsBytes();
      final faces = await StabUtils.getCatFacesFromBytes(bytes);
      expect(faces, isNotNull, reason: 'Cat detection should return a result');
      expect(faces, isNotEmpty, reason: 'Should detect at least one cat');
    });

    testWidgets('extracts eye landmarks from cat', (tester) async {
      if (!await initAppAndFixtures(tester)) return;

      final bytes = await File(testImage!).readAsBytes();
      final faces = await StabUtils.getCatFacesFromBytes(bytes);
      if (faces == null || faces.isEmpty) {
        markTestSkipped('Cat detection returned no faces');
        return;
      }
    ...
    testWidgets('detects cat via project type dispatcher', (tester) async {
      if (!await initAppAndFixtures(tester)) return;
      ...
      expect(
        faces,
        isNotEmpty,
        reason: 'Project type dispatcher should detect cat',
      );
    });

    testWidgets('detects cat in all sample images', (tester) async {
      if (!await initAppAndFixtures(tester)) return;

      for (int day = 1; day <= 4; day++) {
        final path = await getSampleCatPathAsync(day);
        ...
        expect(
          faces,
          isNotEmpty,
          reason: 'Day $day: should detect at least one cat',
        );
      }
    });
  });

  group('Cat Stabilization', () {
  ...
  group('Cat End-to-End Pipeline', () {
    testWidgets('stabilizes image with detected cat face', (tester) async {
      if (!await initAppAndFixtures(tester)) return;
      ...
      final stabilizedFaces = await StabUtils.getCatFacesFromBytes(stabilized!);
